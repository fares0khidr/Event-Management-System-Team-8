...
public class LoginOrganizer {
    public static Organizer loggedInOrganizer = null; // store the logged-in organizer

    public static Scene getLoginScene(Stage stage1) {
        GridPane gridpane = new GridPane();

        Label LoginNewOrganizer = new Label("Login Organizer");
        Label usernameLabel = new Label("Username :");
        Label passwordLabel = new Label("Password :");
        Button LoginOrg = new Button("Login");
        ...
        gridpane.setHgap(5);
        gridpane.setVgap(5);
        gridpane.add(LoginNewOrganizer, 0, 0);
        ...
        gridpane.add(LoginOrg, 1, 3);

        LoginOrg.setOnAction(actionEvent -> {
            String username = usernametxtfield.getText();
            String password = passwordtxtfield.getText();
            Organizer found = null;

            for (Organizer org : Database.organizerList) {
                if (org.getUsername().equals(username) && org.getPassword().equals(password)) { //getusername w pass hatla2eha fel user class
                    found = org;
                    break;
                }
            }
            ...
            if (found != null) {
                ...
                Continue.setOnAction(actionEvent1 -> {
                    Scene OG = OrganizerMenu.getOrganizerMenu(stage1);
                    stage1.setScene(OG);
                });
                ...
                Retry.setOnAction(actionEvent1 -> {
                    Scene RetryScene = LoginOrganizer.getLoginScene(stage1);
